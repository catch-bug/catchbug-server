...
class user
{
  ...
  {
    if ($project !== 0) {
      $query = "SELECT id, project_id, level, language, id_str, last_occ, last_timestamp FROM item WHERE user_id=$this->id and project_id=$project order by id desc";
    } else {
      $query = "SELECT id, project_id, level, language, id_str, last_occ, last_timestamp FROM item WHERE user_id=$this->id order by id desc";
    }
